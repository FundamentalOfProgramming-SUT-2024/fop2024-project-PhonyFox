...
int rankUsersByScore() {
    ...
    while (fgets(line, sizeof(line), file) && count < 100) {
        sscanf(line, "%[^,],%d,%d,%d,%d", users[count].username, &users[count].won_games, &users[count].total_games, &users[count].score, &users[count].gold);
        count++;
    }
    fclose(file);
    ...
    while (1) {
        ...
        for (int i = start_index; i < end_index; i++) {
            ...
            if (strcmp(users[i].username, currentUserLogin) == 0) {
                printw("%d*. %s (Score: %d) (Wins: %d) (Total Games: %d) (Golds: %d)",
                    i + 1, users[i].username, users[i].score, users[i].won_games, users[i].total_games, users[i].gold);
            } else {
                printw("%d. %s (Score: %d) (Wins: %d) (Total Games: %d) (Golds: %d)",
                       i + 1, users[i].username, users[i].score, users[i].won_games, users[i].total_games, users[i].gold);
            }
